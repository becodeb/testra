...
export function ExamEditor({ initialExam }: ExamEditorProps) {
  ...
  }, [allowBackwards, allowReconnect, autoSubmit, blockClipboard, detectFocusLoss, initialExam.id, instructions, questions, questionsToServe, recordDisconnects, requireFullscreen, resultsDisplay, resultsWhen, showProgress, shuffleOptions, shuffleQuestions, status, subject, supervisionLevel, timeLimit, title, violationAction]);
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <Dialog><DialogTrigger asChild><Button type="button" variant="outline" size="sm"><Settings2 data-icon="inline-start" />Configuración</Button></DialogTrigger><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Configuración de la evaluación</DialogTitle><DialogDescription>Definí navegación, tiempo, supervisión y publicación de resultados.</DialogDescription></DialogHeader><div className="grid gap-6 py-2">
                ...
                <section><h3 className="text-sm font-semibold text-ink">Pozo de preguntas</h3><p className="mt-1 text-sm leading-relaxed text-muted">Cargá todas las preguntas que quieras y elegí cuántas recibe cada alumno. El subconjunto se sortea por alumno, así que dos compañeros no reciben las mismas preguntas y no hace falta armar tema A y tema B.</p><div className="mt-3 grid gap-3 sm:grid-cols-[auto_12rem]"><Toggle label="Servir solo una parte del pozo" checked={questionsToServe !== null} onChange={(value) => setQuestionsToServe(value ? Math.min(10, questions.length) : null)} />{questionsToServe !== null ? <Field><FieldLabel htmlFor="exam-serve">Preguntas por alumno</FieldLabel><Input id="exam-serve" type="number" min={1} max={questions.length} value={questionsToServe} onChange={(event) => setQuestionsToServe(Math.max(1, Math.min(questions.length, Number(event.target.value) || 1)))} /></Field> : null}</div>{questionsToServe !== null ? <p className="mt-2 text-sm font-medium text-ink-2">Cada alumno responde {questionsToServe} de {questions.length} preguntas.</p> : null}</section>
