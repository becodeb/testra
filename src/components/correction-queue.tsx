...
export function CorrectionQueue({ initialItems }: { initialItems: CorrectionItem[] }) {
  ...
  async function save(item: CorrectionItem, pointsAwarded: number) {
    ...
    if (response.ok) setItems((current) => current.map((candidate) => candidate.participantId === item.participantId && candidate.questionId === item.questionId ? { ...candidate, pointsAwarded } : candidate));
    else window.alert("No se pudo guardar la corrección");
    setWorkingKey("");
  }

  if (!items.length) return <section className="flex min-h-[55dvh] flex-col items-center justify-center rounded-lg border border-dashed bg-paper p-8 text-center" aria-labelledby="page-title"><p className="mb-2 text-xs font-semibold tracking-[.08em] text-muted uppercase">Sin pendientes</p><h1 id="page-title" className="text-xl font-semibold text-ink">No hay desarrollos por corregir</h1><p className="mt-2 max-w-md text-sm leading-relaxed text-muted">Las respuestas automáticas se corrigen siempre en el servidor. Los desarrollos entregados aparecerán acá.</p><a href="/tomas" className="mt-5 inline-flex h-9 items-center rounded-md border bg-white px-4 text-sm font-semibold text-ink-2 hover:bg-inset">Ver tomas anteriores</a></section>;

  return (
    <section className="grid gap-5" aria-labelledby="page-title" data-correction-ready={ready}>
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="mb-1 text-xs font-semibold tracking-[.08em] text-muted uppercase">Corrección manual</p><h1 id="page-title" className="text-2xl font-semibold text-ink">Desarrollos</h1><p className="mt-1 text-sm text-muted">{pending ? `${pending} respuesta${pending === 1 ? "" : "s"} pendiente${pending === 1 ? "" : "s"}.` : "Todo corregido. Podés revisar y ajustar los puntajes."}</p></div><span className="mono-number rounded-md border bg-paper px-3 py-2 text-sm font-semibold">{pending}/{items.length} pendientes</span></div>
...
function CorrectionRow({ item, working, onSave }: { item: CorrectionItem; working: boolean; onSave: (item: CorrectionItem, points: number) => Promise<void> }) {
  ...
  return (
    <article className="grid gap-4 p-4 md:grid-cols-[12rem_1fr_9rem] md:p-5">
      <div><p className="font-semibold text-ink">{item.studentName}</p><p className="mt-1 text-xs text-muted">{item.runTitle}</p><a href={`/tomas/${encodeURIComponent(item.runId)}`} className="mt-2 inline-block text-xs font-semibold text-brand hover:underline">Ver toma</a></div>
