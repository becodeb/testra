import { BrainCircuit, CalendarDays, CheckCircle2, ChevronRight, LoaderCircle, Users } from "lucide-react";
...
interface ResultsSnapshot { run: { id: string; title: string; code: string; status: "lobby" | "running" | "ended"; created_at: number; results_published_at: number | null; classroom_course_id: string | null; classroom_coursework_id: string | null }; questionCount: number; totalPoints: number; poolSize: number; participants: Array<Record<string, string | number | null>>; }
...
function AiReportCard({ report, kind }: { report: AiEnvelope; kind: "run" | "person" }) {
  ...
  const benign = Array.isArray(data.benignExplanations) ? data.benignExplanations as string[] : [];
  const reviewLabel: Record<string, string> = { high: "prioridad alta", medium: "prioridad media", low: "prioridad baja" };
  return <aside className="m-5 rounded-lg border border-brand/20 bg-brand-soft/40 p-5">
    <div className="flex items-center gap-2"><BrainCircuit className="size-5 text-brand" /><h3 className="font-semibold text-brand-deep">Lectura orientativa para el docente</h3></div>
    <p className="mt-2 text-xs leading-5 text-muted">La IA organiza señales para ahorrar tiempo. No califica, no sanciona y no puede determinar por sí sola si alguien copió.</p>
    <p className="mt-3 text-sm leading-6 text-ink-2">{String(data.summary ?? "Análisis generado.")}</p>
    {kind === "run" && candidates.length ? <div className="mt-4 grid gap-2">{candidates.map((candidate) => <div key={String(candidate.participantId)} className="rounded-md bg-white p-3 text-sm"><strong>{String(candidate.name)}</strong><span className="ms-2 text-xs font-semibold text-brand">{reviewLabel[String(candidate.risk)] ?? "para revisar"}</span><p className="mt-1 leading-5 text-muted">{Array.isArray(candidate.reasons) ? candidate.reasons.join(" · ") : ""}</p></div>)}</div> : null}
    {kind === "person" && evidence.length ? <section className="mt-4"><h4 className="text-sm font-semibold text-ink">Qué ocurrió</h4><ul className="mt-2 list-disc space-y-1 ps-5 text-sm text-ink-2">{evidence.map((item) => <li key={item}>{item}</li>)}</ul></section> : null}
    {kind === "person" && benign.length ? <section className="mt-4"><h4 className="text-sm font-semibold text-ink">Explicaciones normales posibles</h4><ul className="mt-2 list-disc space-y-1 ps-5 text-sm text-muted">{benign.map((item) => <li key={item}>{item}</li>)}</ul></section> : null}
    {data.recommendation ? <p className="mt-4 rounded-md bg-white p-3 text-sm leading-6 text-ink-2"><strong>Qué conviene revisar:</strong> {String(data.recommendation)}</p> : null}
    <p className="mt-4 border-t pt-3 text-xs leading-5 text-muted">{String(data.caveat ?? "Es una lectura orientativa. El docente debe revisar el contexto antes de tomar una decisión.")}</p>
  </aside>;
}
