...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  try {
    const input = z.object({ participantId: z.string().min(1), questionId: z.string().min(1), pointsAwarded: z.number().min(0), feedback: z.string().max(4000).optional(), rubricScores: z.record(z.string(), z.number().min(0)).optional() }).parse(await readJson(request));
